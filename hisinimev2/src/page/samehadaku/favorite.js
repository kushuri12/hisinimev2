...
export function favorite() {
  setTimeout(async () => {
    ...
    document.title = "Favorite - Samehadaku Mode"
...
async function renderFavorites(container, navigateTo) {
  // Bersihkan container
  container.innerHTML = '<h2 class="text-xl text-purple-300 font-bold mb-4">Daftar Favorit (Samehadaku)</h2>';
  ...
  for (let id of favs) {
    try {
      ...
      const result = await fetchFromSource("Samehadaku", `anime/${encodeURIComponent(id)}`);
      ...
      episodeSpan.textContent = `Episode: ${data.episodes}`;
      ...
      li.onclick = () => navigateTo(`/anime/samehadaku/detail?id=${id}`);
      ul.appendChild(li);
