...
const SamehadakuDetail = () => {
  ...
  const loadAnimeDetail = async () => {
    try {
      ...
      if (data?.data?.data) {
        ...
        setAnime(animeData);
        setEpisodes(animeData.episodeList || []);
        document.title = `${animeData.title || animeData.english || animeData.synonyms} - HisiNime v2`;
      }
    } catch (error) {
  ...
  const checkFavoriteStatus = async () => {
    ...
    const isFav = favs.some(fav => fav.id === id && fav.source === "Samehadaku");
  ...
  const handleEpisodeClick = (episode) => {
    navigate(`/anime/samehadaku/watch?id=${id}&episode=${episode.episodeId}`);
  };
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <div className="flex items-center justify-between mb-6 p-4">
        ...
        <div className="text-center flex-1">
          <h1 className="text-gradient font-bold text-xl md:text-2xl">
            {anime.title || anime.english || anime.synonyms}
          </h1>
          <p className="text-sm text-purple-400 font-semibold">Samehadaku</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 px-4 mb-6">
        ...
        <div className="lg:col-span-1">
          <div className="card p-4">
            <img
              src={anime.poster}
              alt={anime.title || "tidak diketahui"}
              className="w-full rounded-lg shadow-lg mb-4"
            />
            <button
              onClick={handleFavoriteToggle}
              className={`group relative w-full overflow-hidden rounded-xl bg-gradient-to-r ${
                isFavorited ? 'from-red-500 to-pink-500' : 'from-purple-500 to-purple-600'
          ...
          <div className="card p-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
              <div className="flex justify-between">
                ...
                <span className="text-white">{anime.japanese || 'N/A'}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-white">{anime.score?.value || "N/A"}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-400">Studio:</span>
                <span className="text-white">{anime.studios || 'N/A'}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-white">{anime.aired || 'N/A'}</span>
              </div>
            </div>
          </div>
          ...
          <div className="card p-4">
            ...
            <div className="flex flex-wrap gap-2">
              {anime.genreList?.map(g => (
                <span key={g.title} className="badge">{g.title}</span>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <p className="text-white font-medium text-sm">Episode {ep.title}</p>
